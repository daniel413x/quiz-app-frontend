...
interface QuizFormFieldsProps {
  ...
  isAnsweredIncorrectly: boolean | null;
  answers: Answer[];
        ...
        <FormItem>
          ...
          >
            {answers.map((answer, i) => {
              const isWrongAnswer = answer.id === answerId && isAnsweredIncorrectly && !isAnsweredCorrectly;
              const isCorrectAnswer = answer.id === answerId && isAnsweredCorrectly;
              return (
                <Label
                ...
                >
                  ...
                  <span
                    className={cn(buttonVariants({ variant: 'outline', className: 'relative flex flex-row-reverse w-full text-wrap justify-start border border-black/75  dark:border-gray-700 shadow-sm ps-16 pe-5 py-[1.375rem] cursor-pointer group h-max ' }), {
                      ...
                      'opacity-50': isAnsweredCorrectly && answerId !== answer.id,
                    })}
                  >
                    ...
                    >
                      <RenderedMarkdown html={answer.answer} />
                    </span>
                    <RadioGroupItem
                      checked={answer.id === form.watch('answerId')}
                      value={answer.id}
                      id={answer.id}
